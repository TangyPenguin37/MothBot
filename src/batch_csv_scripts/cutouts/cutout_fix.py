...
for file in tqdm(allFiles):
    ...
    if not ("batch1" in file and "rear" in file and "right" in file) and not (
            "batch2" in file and "rear" in file and "left" in file):
        continue

    if "batch1" in file and "rear" in file and "right" in file:
        filename = os.path.splitext(os.path.basename(file))[0]
        if not filename in batch1_rear_right:
            continue

    if "batch2" in file and "rear" in file and "left" in file:
        filename = os.path.splitext(os.path.basename(file))[0]
        if not filename in batch2_rear_left:
            continue
    ...
    if not cv2.imwrite(os.path.join(cutoutDir, file), result):
        ...
        print(f"Could not write image {file} to file!")
...
def load_data(arm_balance=None, balance_train_only=False, test_size=0.2):
    ...
    if balance_train_only:
        ...
        x_train, y_train = imblearn.over_sampling.SMOTE().fit_resample( # type: ignore
            x_train, y_train)
        return x_train, x_test, y_train, y_test

    # return train_test_split(*balance(x, y, arm_balance),
    #                         test_size=test_size,
    #                         stratify=y)

    return train_test_split(*imblearn.over_sampling.SMOTE().fit_resample(x, y),
                            test_size=test_size,
                            stratify=y)
...
if __name__ == "__main__":
    data = load_data(arm_balance=0.5, balance_train_only=True)
